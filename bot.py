...
async def process_sub_link(message: Message, state: FSMContext):
    ...
    sub_link = "" if message.text.strip() == "0" else message.text.strip()
...
async def admin_payments(callback: CallbackQuery):
    ...
    if not payments:
        ...
        lines = [f"💳 *پرداخت‌های معلق ({len(payments)} عدد):*\n"]
...
async def admin_set_support(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"🆘 *تنظیم پشتیبانی*\n\n"
        f"آیدی فعلی: `{current or 'تنظیم نشده'}`\n\n"
        f"یوزرنیم یا لینک پشتیبانی را وارد کنید:\nمثال: `@support_username`",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="❌ انصراف", callback_data="admin:back")]
        ])
...
async def del_config_cmd(message: Message):
    if not is_admin(message.from_user.id):
        await message.answer("⛔ شما دسترسی ندارید."); return
    parts = message.text.split(maxsplit=1)
    if len(parts) < 2:
        await message.answer("❌ استفاده صحیح:\n`/delconfig <id>`", parse_mode="Markdown"); return
    try:
...
async def set_balance_cmd(message: Message):
    ...
    if len(parts) < 3:
        await message.answer("❌ استفاده صحیح:\n`/setbalance <user_id> <مقدار>`", parse_mode="Markdown"); return
    try:
...
async def add_balance_cmd(message: Message):
    ...
    parts = message.text.split()
    if len(parts) < 3:
        await message.answer("❌ استفاده صحیح:\n`/addbalance <user_id> <مقدار>`", parse_mode="Markdown"); return
    try:
        uid = int(parts[1]); amount = int(parts[2])
        if not get_user(uid):
            await message.answer("❌ کاربر پیدا نشد."); return
        ...
        await bot.send_message(uid, f"💰 مبلغ *{amount:,}* تومان به موجودی شما افزوده شد.", parse_mode="Markdown")
    except ValueError:
        await message.answer("❌ مقادیر وارد شده معتبر نیستند.")
...
async def delete_user_cmd(message: Message):
    ...
    parts = message.text.split()
    if len(parts) < 2:
        await message.answer("❌ استفاده صحیح:\n`/deleteuser <user_id>`", parse_mode="Markdown"); return
    try:
        uid = int(parts[1])
        delete_user(uid)
...
async def handle_photo(message: Message):
    ...
    inline_kb  = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="تأیید ✅", callback_data=f"approve:{payment_id}"),
        InlineKeyboardButton(text="رد ❌",    callback_data=f"reject:{payment_id}"),
    ]])
...
async def approve_payment(callback: CallbackQuery):
    if callback.from_user.id != ADMIN_ID:
        await callback.answer("⛔ شما دسترسی ندارید.", show_alert=True); return
    payment_id = int(callback.data.split(":")[1])
    payment = get_payment(payment_id)
    if not payment:
        await callback.answer("پرداخت یافت نشد.", show_alert=True); return
    if payment[3] != "pending":
        await callback.answer("این پرداخت قبلاً پردازش شده است.", show_alert=True); return
    user_id = payment[1]
